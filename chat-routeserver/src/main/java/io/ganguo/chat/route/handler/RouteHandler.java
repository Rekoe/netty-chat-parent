...
public class RouteHandler extends IMHandler<IMRequest> {
	...
	private void loginChannel(IMConnection connection, IMRequest request) {
		...
		// boolean isSuccess = userService.authenticate(login.getUin(),
		// login.getAuthToken());
		...
		if (true) {
			...
			/*
			 * // 是否已经登录进来了，踢下线 ClientSession old =
			 * ClientSessionManager.getInstance().get(login.getUin()); if (old != null &&
			 * old.getConnection() != connection) { kick(old.getConnection(), request); } //
			 * 绑定用户UIN到connection中 ClientSession session = new ClientSession(login,
			 * connection);
			 */
	...
	private void login(IMConnection connection, IMRequest request) {
		...
		if (isServer) {
			/*
			 * header.setHandlerId(getId()); header.setCommandId(Commands.LOGIN_SUCCESS);
			 * resp.setHeader(header); connection.sendResponse(resp);
			 */
